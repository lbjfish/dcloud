...
public class CorsFilterConfiguration implements Filter {
    ...
    public void doFilter(final ServletRequest servletRequest,
        ...
        if (decorateRequest) {
            CorsFilterConfiguration.decorateCORSProperties(request, requestType);
        }
        switch (requestType) {
            case SIMPLE:
    ...
    public void init(final FilterConfig filterConfig) throws ServletException {
        ...
        parseAndStore(DEFAULT_ALLOWED_ORIGINS, DEFAULT_ALLOWED_HTTP_METHODS,
                "Content-Type,X-Requested-With,accept,Origin,Access-Control-Request-Method,Access-Control-Request-Headers,Authorization,hjsTokenKey,Cache-Control,Pragma,If-Modified-Since", DEFAULT_EXPOSED_HEADERS,
...
//            String configAllowedHttpHeaders ="Content-Type,X-Requested-With,accept,Origin,Access-Control-Request-Method,Access-Control-Request-Headers,Authorization,X-Zkcf-Token,X-Zkcf-PageSize,X-Zkcf-PageNum";
...
//        }
    ...
    protected void handlePreflightCORS(final HttpServletRequest request,
        ...
        String accessControlRequestMethod = request.getHeader(
                CorsFilterConfiguration.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_METHOD);
        ...
        String accessControlRequestHeadersHeader = request.getHeader(
                CorsFilterConfiguration.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_HEADERS);
        ...
        if (preflightMaxAge > 0) {
            response.addHeader(
                    CorsFilterConfiguration.RESPONSE_HEADER_ACCESS_CONTROL_MAX_AGE,
                    String.valueOf(preflightMaxAge));
        }

        // Section 6.2.9
        response.addHeader(
                CorsFilterConfiguration.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_METHODS,
                accessControlRequestMethod);

        // Section 6.2.10
        if ((allowedHttpHeaders != null) && (!allowedHttpHeaders.isEmpty())) {
            response.addHeader(
                    CorsFilterConfiguration.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_HEADERS,
        ...
        switch (corsRequestType) {
            case SIMPLE:
                request.setAttribute(
                        CorsFilterConfiguration.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST,
                        Boolean.TRUE);
                request.setAttribute(CorsFilterConfiguration.HTTP_REQUEST_ATTRIBUTE_ORIGIN,
                        request.getHeader(CorsFilterConfiguration.REQUEST_HEADER_ORIGIN));
                request.setAttribute(
                        CorsFilterConfiguration.HTTP_REQUEST_ATTRIBUTE_REQUEST_TYPE,
            ...
            case PRE_FLIGHT:
                ...
                request.setAttribute(
                        CorsFilterConfiguration.HTTP_REQUEST_ATTRIBUTE_REQUEST_HEADERS, headers);
                break;
            case NOT_CORS:
                request.setAttribute(
                        CorsFilterConfiguration.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST,
